-- Use to run mysql db docker image, optional if you''e not using a local mysqldb
...
CREATE USER 'recipe_dev_user'@'localhost' IDENTIFIED BY 'geo';
CREATE USER 'recipe_prod_user'@'localhost' IDENTIFIED BY 'geo';
CREATE USER 'recipe_dev_user'@'%' IDENTIFIED BY 'geo';
CREATE USER 'recipe_prod_user'@'%' IDENTIFIED BY 'geo';
